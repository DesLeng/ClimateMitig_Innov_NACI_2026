...
*== Number of HVI and all CCMT inventions(1995-2025) 
*== Share of each CCMT technology classes in all CCMT 1995-2000 & 2010-2025
use  "$datapath/Final_database/mitigation_inv_tech_inventor_ctry_year",clear
keep if publn_year <2026 & publn_year >1994
...
egen hvi_CCMT_1995_2025 =sum(world_hvi_CCMT) if publn_year <=2025 & publn_year >=1995, by(techno)
*=Total number of all CCMT inventions patented worldwide for the period 1995-2025,
*=by CCMT technology fields
egen inv_CCMT_1995_2025 =sum(world_inv_CCMT) if publn_year <=2025 & publn_year >=1995, by(techno)
keep techno hvi* inv_*
duplicates drop
egen hvi_Y02_1995_2025 = max(hvi_CCMT_1995_2025)
egen inv_Y02_1995_2025 = max(inv_CCMT_1995_2025)
*=Share of hvi CCMT inventions in all CCMT inventions patented worldwide for the period 1995-2025,
gen sh_hvi_in_inv_CCMT_9517 = 100 * hvi_Y02_1995_2025 / inv_Y02_1995_2025
sort techno
duplicates drop
export excel "$droppath/Analysis/SELECT_NatEn/Main/Table1_NbText.xlsx", replace firstrow(variables)


*== FIGURE1: Evolution of global high-value climate change mitigation technology inventions from 1995-2025
*== Evolution HVI CCMT vs All technologies (1995-2025): Index 1995
use  "$datapath/Final_database/mitigation_inv_tech_inventor_ctry_year",clear
keep if publn_year <2025 & publn_year >1994
...
*== Average growth rate by CCMT tech fields (1995-2012) vs (2013-2025)
use  "$datapath/Final_database/mitigation_inv_tech_inventor_ctry_year",clear
keep if publn_year <2025 & publn_year >1993
...
*Growth rate 2013-2025
use  "$datapath/Final_database/mitigation_inv_tech_inventor_ctry_year",clear
keep if  publn_year <=2025 & publn_year >=2013
keep technology tech_name publn_year world_hvi_CCMT world_hvi_all
sort technology publn_year
duplicates drop
bys technology: gen growthCCMT = 100*(world_hvi_CCMT[_n] - world_hvi_CCMT[_n-1]) / world_hvi_CCMT[_n-1] 
keep publn_year growth* technology tech_name
duplicates drop
egen m_growthCCMT1317 = mean((growthCCMT) * ( publn_year <=2025 & publn_year >=2013)), by(technology)
 ...
 *== FIGURE3: Yearly patent applications across sub-sectors
*== FIGURE3a: ENERGY: Hydrogen, Renewables & Enabling technologies (Storage)
...
*== FIGURE3b: ENERGY SUB-CATEGORY SOLAR: Polycrystalline PV, Organic PV, CUInSe2
...
*== FIGURE3c: BUILDINGS: Efficient home appliances, Heating
...
*== FIGURE3d: MANUFACTURING: Chemical Industry, Industrial and consumer goods,
...
keep field publn_year world_hvi_CCMT_field world_CCMT_Index95
export excel "$droppath/Analysis/SELECT_NatEn/Main/Figure4d.xlsx", replace firstrow(variables)
*== FIGURE3e: CARBON CAPTURE AND STORAGE (CCS): CO2 CCS & other GHG CCS
use  "$datapath/Final_database/CCMT_SubCat_inv_by_fields_inventor_ctry_year.dta" , clear
...
*== FIGURE3f: TRANSPORT: Air transport, Maritime, Rail, Electric Vehicles (EVs)
...
gen world_CCMT_Index95= world_hvi_CCMT_field / world_CCMT1995
keep field  publn_year world_hvi_CCMT_field world_CCMT_Index95
 export excel "$droppath/Analysis/SELECT_NatEn/Main/Figure3f.xlsx", replace firstrow(variables)
 
 	
*== FIGURE4: Top-10 inventor countries in CCMT
*== % of CCMT  inventions made by TOP 10 inventor countries (2011-2025) vs(2000-2010)
* 2011-2025
use "$datapath/Final_database/mitigation_inv_tech_inventor_ctry_year", clear
keep if publn_year >=2011 & publn_year <=2025
keep if techno =="Y02"
keep techno invt_iso invt_name publn_year income ///
nb_hvi_CCMT nb_hvi_all world_hvi_CCMT world_hvi_all
duplicates drop
*Identification TOP 10 inventors 2011-2025
...
sort cat rank 
save "$droppath/Analysis/SELECT_NatEn/Main/Figure4a.dta", replace
* 2000-2010
use "$datapath/Final_database/mitigation_inv_tech_inventor_ctry_year", clear
keep if publn_year >=2000 & publn_year <=2010
keep if techno =="Y02"
keep techno invt_iso invt_name publn_year income ///
nb_hvi_CCMT nb_hvi_all world_hvi_CCMT world_hvi_all
duplicates drop
...
mmerge invt_name using "$droppath/Analysis/SELECT_NatEn/Main/Figure4a.dta"
keep if cat !=.
browse
order cat rank invt_name sh_worldHVI_CCMT* sh_worldHVI_ALL*
sort cat rank
drop _m
export excel "$droppath/Analysis/SELECT_NatEn/Main/Figure4.xlsx", replace firstrow(variables)

*== FIGURE5: Climate change mitigation technology specialisation from 2000-2025
*== Specialization in CCMT for TOP 10 inventor countries (2011-2025) vs (2000-2010)
*2011-2025
use  "$datapath/Final_database/mitigation_inv_tech_inventor_ctry_year",clear
keep if publn_year <=2025 & publn_year >=2011
keep if techno =="Y02"
keep techno invt_iso invt_name publn_year income ///
nb_hvi_CCMT nb_hvi_all world_hvi_CCMT world_hvi_all
duplicates drop
...
*2000-2010
use  "$datapath/Final_database/mitigation_inv_tech_inventor_ctry_year",clear
keep if publn_year <=2010 & publn_year >=2000
keep if techno =="Y02"
keep techno invt_iso invt_name publn_year income ///
nb_hvi_CCMT nb_hvi_all world_hvi_CCMT world_hvi_all
duplicates drop
...
duplicates drop
mmerge invt_iso using "$droppath/Analysis/SELECT_NatEn/Main/Figure5a.dta", unmatched(using)
drop _m
order invt* spe*
export excel "$droppath/Analysis/SELECT_NatEn/Main/Figure5.xlsx", replace firstrow(variables)


*== FIGURE6: Source and destination of transferred climate change mitigation technologies from 2011-2025
*== Matrix transfers income (mitigation) 2011-2025
